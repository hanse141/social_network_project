...
public class ServerConnection implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            while (isRunning) {
        		...
        		if(command != null && command.substring(0, 2).equals("gd")) {
        			if(Client.isLoggedIn()) {
        			//guiThread.join();
        				Client.gui.updateMessages(convertToMessages(command));
    ...
    public static ArrayList<String> convertToConversations(String input) {
    	try {
    		...
    		String chats = input.substring(input.indexOf("Chats") + 7, input.indexOf("Messages<") - 3);
